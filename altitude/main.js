(function () {
  ...
  /* --------------------------------------------------------------
     Showcase carousel ("Designed for clients who expect more")
     — only present on the About page.
     The track is a native scroll-snap container, so it stays usable
     with swipe / trackpad / keyboard even if this script never runs;
     the arrows and dots are the progressive enhancement.
  -------------------------------------------------------------- */
  ...
  if (carousel) {
    ...
    if (track && slides.length) {
      ...
      function goToSlide(i) {
        var target = slides[Math.min(Math.max(i, 0), slides.length - 1)];
        if (target) track.scrollTo({ left: target.offsetLeft - slides[0].offsetLeft, behavior: reduceMotion ? 'auto' : 'smooth' });
      }
      ...
      function nearestSlide() {
        ...
        slides.forEach(function (slide, i) {
          var dist = Math.abs((slide.offsetLeft - origin) - pos);
      ...
      function syncCarousel() {
        ...
        // Buttons reflect the real scroll position, not just the index,
        // so they disable exactly at the ends of the track.
        var atStart = track.scrollLeft <= 1;
        var atEnd = track.scrollLeft >= track.scrollWidth - track.clientWidth - 1;
        if (prevBtn) prevBtn.disabled = atStart;
        if (nextBtn) nextBtn.disabled = atEnd;
        carousel.classList.toggle('is-end', atEnd);
      }

      if (prevBtn) prevBtn.addEventListener('click', function () { goToSlide(nearestSlide() - 1); });
      if (nextBtn) nextBtn.addEventListener('click', function () { goToSlide(nearestSlide() + 1); });
      ...
      dots.forEach(function (dot, d) {
        ...
        dot.setAttribute('aria-selected', String(d === 0));
      });
      syncCarousel();
    }
  }
